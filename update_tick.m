...
%%last tick.
function [ old_position, position, marble_angle, linear_velocity, linear_acceleration, angular_velocity, angular_acceleration] = update_tick(mass,fps,position,marble_angle,gravity, linear_velocity, linear_acceleration, angular_velocity, angular_acceleration, col_occur, radius, ramp_list,collided_ramp)
    ...
    if col_occur
       ...
       temp_theta = atan(ramp_direction(2)/ramp_direction(1));
        ...
        if collided_ramp ~= 5 %non flat surface
            linear_acceleration(1) = cos(temp_theta)*(-sin(temp_theta)*mass*gravity)/(mass-0.4*radius*mass);
            linear_acceleration(2) = sin(temp_theta)*(-sin(temp_theta)*mass*gravity)/(mass-0.4*radius*mass);
            angular_acceleration = sin(temp_theta)*mass*gravity/(mass*radius-0.4*radius^2);
        end
